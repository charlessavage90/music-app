# `NEXT.md` — superseded status blocks

**Role: HISTORICAL. Frozen, and never edited.** This is the archive of [`NEXT.md`](NEXT.md)'s
superseded status blocks, split out on 2026-09-05 when the status document had reached 2,063
lines — 1,738 of them this. Each block below was the live top block of `NEXT.md` on the date
in its heading, and was demoted when the next `closeout` wrote a new one.

**Do not read this for status.** `NEXT.md` owns status, and nothing here is current by
construction: every block below has been superseded at least once. The supersession
annotations inside them were accurate when written and are not maintained.

**Do not read it for standing constraints either.** Anything from these blocks that still
binds — a "must not be reverted" claim, a closed decision, a deferral — belongs in `NEXT.md`'s
own registries (*Closed*, *Must not be changed*, *Deferred, with conditions*). If something
here looks load-bearing and is not in one of those, that is a distillation defect in
`NEXT.md`: fix it forward there, and still do not edit this file.
...
### Deferred, with conditions (as of 2026-09-24)
