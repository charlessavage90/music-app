...
> **⚠ AMENDED 2026-07-23/24 — twelve amendments (A1–A12); A1–A7 before any arm ran, A8–A9 on the A0 gate result, A10 fixes §5's sample before any label, A11 resolves the fame proxy on the P4 pilot result, A12 stops C6 gating because A11 inverted what it measures. No factorial arm has run.**
...
| Popularity-**percentile** machinery | ✘ not built *(at writing)* | Nothing in `api/src` computes popularity percentiles (`evaluation.py`'s only percentile function is over *degree*). Prior probes computed it ad hoc (`tail_probe.py`). To be built in the harness — see prerequisite P6 for the tie-handling rule. **STATUS CORRECTION 2026-07-24 (not an amendment — no design changed):** it *was* built, at execution-order step 2, in `builder/analysis/2026-07-23-track2-sweep/mirror.py` (`MirrorContext.build`, commit `2dedefd`), and passed the byte-identity gate. **P6 is discharged**; three documents carried it as open until the 2026-07-24 doc audit. `api/src` remains percentile-free by design — shipped code is not edited before adoption. |
...
| ~~P6~~ | ~~Fix percentile tie-handling: average rank over the popularity array, computed once at harness start~~ **DISCHARGED 2026-07-23 at execution-order step 2 — do not rebuild.** Implemented in `builder/analysis/2026-07-23-track2-sweep/mirror.py`, `MirrorContext.build` (average rank over N, ties averaged, plus A3's `jump_scale_pctl` mean-matching ratio); commit `2dedefd`, covered by the byte-identity gate. **Recorded as open until the 2026-07-24 doc audit** — see the execution log's status correction. | Determinism; `np.argsort` alone is order-dependent among ties |
...
Twelve amendments — six prompted by the `ml-graph-analyst` protocol review, one (A7) by P1's answer, two (A8, A9) by the A0 gate result and its review, one (A10) by deriving §5's sample against the repo, **and two (A11, A12) on 2026-07-24 — A11 by the P4 proxy-pilot result, A12 by A11's own knock-on** (both proxies fired; these postdate the pilot exactly as A8/A9 postdate the A0 gate — **no factorial/sweep arm has run**)
([`../findings/2026-07-23-track2-protocol-analyst-review.md`](../findings/2026-07-23-track2-protocol-analyst-review.md)).
