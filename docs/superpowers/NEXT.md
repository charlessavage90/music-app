...
| **The Deezer id map covers only the adopted population** | **Before an `ALG-B` artifact is ever served.** `deezer_artist_ids_20260802.json` was extracted over the adopted graph's 74,193 artists. An artist outside it gets no id and falls back to name search — a coverage gap, never a wrong answer, which is why this is *not* the "one list, any archive" hazard repeated: a Deezer id is a property of an artist, while a drop list is a decision about a population. Re-extract with `builder/analysis/2026-08-02-dsp-ids/dsp_ids.py` (~2 min, offline). |
