...
| **Adding `analysis` to builder's `testpaths`** | **Deferred by the owner 2026-07-30.** Every analysis test passes, so it would work cleanly, but it changes what every builder `pytest` run collects. **Revisit if any `TAS-` test needs to gate a merge, or at the closeout that retires the `TAS-` probe.** **⚠ THE CONDITION HAS FIRED.** Task 8 landed 2026-08-01 and **the closeout that retires the `TAS-` probe was run the same evening** — this is the moment the row named. Re-checked at that closeout: `pytest analysis/ -q` passes (read the count off the command, not this row). It is **the owner's deferral to resolve, not a session's**, so it is reported fired rather than actioned — but it is no longer waiting on anything. Until then `TAS-` tests run only when invoked explicitly. *(This row carried a count of "34" until 2026-07-30; it was stale, and the count is not this document's to own — read it off `pytest analysis/ -q`.)* |
