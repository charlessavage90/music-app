...
| ⚠ **One list, applied to any archive** | **Before anything is built from the candidate (`ALG-B`) archive.** The wiring applies today's frozen list regardless of which archive is read, and that list covers only ~a third of the candidate population — so a candidate build would half-apply it, silently, which is the exact asymmetry a cross-archive comparison must not contain. Needs a per-archive list or a guard that refuses rather than half-applies. |
