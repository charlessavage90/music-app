...
| Finding | Condition |
|---|---|
| **`docs-lint` checks 4, 5 and 6 were never observed to run at the stage-2 closeout** — the script times out partway through check 3 (relative-link resolution) on this machine, in both a foreground and a background run. **Checks 1 and 2 were seen to pass after the one hard failure was fixed**; the `doc-auditor` separately verified that the new documents' relative links resolve. ⚠ **The auditor was told checks 4–6 had completed, which overstated what had actually been seen** — recorded rather than left as a silent gap. | **At the next closeout that touches `docs/`**: run `scripts/docs-lint.sh` with a timeout above two minutes and confirm checks 4, 5 and 6 (plans claiming to be live, duplicate bare identifiers, restated figures). **Check 6 is the one that matters most here**, because stage 2's closeout fixed four restated figures by hand and nothing mechanical has confirmed there are no others. |
