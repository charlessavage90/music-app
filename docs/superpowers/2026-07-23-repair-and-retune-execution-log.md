...
| Parked | Trigger |
|---|---|
| **BoilTheFrog source review**, reshaped to one targeted question — *does it apply any popularity/fame term in path selection, or is it plain shortest-path?* | Path-quality work resumes; run it at the **front**, before the rescale probe is designed. Two bounds: it may return **mechanisms, never numbers** (its graph is Spotify related-artists, ours is ListenBrainz-derived and mutual-kNN filtered — a quantity from one is not a quantity about the other), and the report must state the repo's licence. Note the app **cannot be run** — the alpha design §1 records that its data source was deprecated — so the *behavioural* half of `WHAT-GOOD-LOOKS-LIKE` value 9 is unresolvable by anyone, and the value does not depend on the reference being accurate. **✅ DISCHARGED 2026-07-27** — run *ahead of its trigger*, at the owner's direct request, while path work remained paused. Record: `findings/2026-07-27-boilthefrog-source-review.md` (`BTF-`); figures `../../builder/analysis/2026-07-27-boilthefrog-reconstruction/`. **Answer to the targeted question: yes, and it is the *entire* cost function** — `1 + \|Δpopularity\|`, with similarity priced at zero and used only to decide which edges exist. **Both bounds were honoured**: no quantity was carried across the two graphs, and the licence was checked — **there isn't one**, which forbids copying upstream code and lands on this review's own `BTF-7` suggestion. ⚠ **One clause of this trigger is now OVERTURNED.** "The *behavioural* half of value 9 is unresolvable by anyone" assumed the app must be run. It need not be: the repository **commits its own crawl data**, so the graph and router can be reconstructed and the behaviour simulated. Value 9's endpoint-tracking clause is now verified (`BTF-2`) and value 2's bypass clause refuted (`BTF-3`) — the latter is a *simulation* of user clicks, and `BTF-3` states that limit itself. The rest of the trigger's reasoning stands. |
