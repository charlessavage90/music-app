...
| 10 | `FRO-4` | ⏳ **STEP LANDED by `RMD-13`, stage 3 (PR #32); it EXECUTES at the Track C cutover** — `infra/README.md` §8a. Not closed until then, and running it earlier is worthless. **No step anywhere proves the SPA fallback works**, and §8 contains no authenticated request to the site at all. | It proves the gate rejects; it never proves it admits. `TR-5` is the finding two reviewers found independently and it would ship unverified. |
