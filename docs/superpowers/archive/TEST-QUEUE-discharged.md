...
Split out 2026-09-05, when `TEST-QUEUE.md` had reached 1,996 lines of which **93% was
discharged history** and one entry was live. Same shape, same cause and same fix as
[`../NEXT-ARCHIVE.md`](../NEXT-ARCHIVE.md): a live document accumulating its own past because
nothing said where the past should go.

**What an entry here still tells you**, and it is the reason none of this is deleted: what the
owner actually pressed, in his own words, and what it found. Several of this project's turns
were decided here — most consequentially the 2026-09-01 report that fired the revert criterion
on the extended map. **Read it as evidence about the past, never as work.**

**How to tell a live entry from a discharged one**, since both keep their original heading: an
entry is live only if its **topmost** heading says so. A discharged entry keeps its old
`QUEUED` heading *beneath* its `DONE` one. Greping for `QUEUED` is what made six consecutive
