# Archive — "nothing to test" entries from the use-the-app queue

**Role: HISTORICAL. Do not cite, do not act on, and do not add to.** Retained so the record
can be audited, not because anything here is owed.

These are the **40 entries that queued nothing.** Each says some version of *"nothing to
exercise, nothing is running, a journey you build now is identical to yesterday's"*, followed
by a plain-language account of what that session did. **No entry here asks the owner to press
anything, and none records anything he exercised.**

**Why they existed, and why they stopped.** `TEST-QUEUE.md` used to require one entry per
...
rather than narrative. A rule that can silently relocate a discharge is the wrong rule.

**What is not lost.** Every session's reasoning lives in its retained execution log, and
`docs/README.md` maps them. Nothing here is the sole record of anything — checked before the
move, including `G3-F2`, whose falsification is recorded in four other documents.
