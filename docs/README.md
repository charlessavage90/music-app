...
| `superpowers/findings/2026-07-25-bypass-depth-use-run.md` | **AUTHORITATIVE for its own measurements — the owner's own 100-press use of the shipped app, 2026-07-25.** Dogfooding, not an experiment: one pair (Bob Dylan → Metallica), one listener, one sitting, nothing scored or pre-registered, and §1 states its three protocol defects up front. **Owns** the seven-unfamiliar-artists table with hand-read Spotify monthly listeners (`BYP-1`), the 29-press wait for the first (`BYP-2`), the **372k-monthly-listener floor across all 100 presses** (`BYP-3`), and the path-length trajectory (`BYP-4`) showing length **oscillating** — spikes to 12/13/11 cards collapsing back to 5–6 — with `BYP-5` recording that after 100 accumulated exclusions the output is qualitatively where it started. `BYP-4`/`BYP-5` are measured **against a stated requirement**: `WHAT-GOOD-LOOKS-LIKE` value 2 asks bypass to *progressively* lengthen and add novelty. `BYP-6` (novelty rides the length spikes) and `BYP-7` (the router flips between a many-small-steps regime and a few-big-jumps regime) are labelled **inference** with falsifiers. `BYP-8` raises — as a hypothesis only, explicitly **not** an explanation of `R0` — that Track 2/2F sampling at depths 10/15/20 may have measured an oscillating signal at arbitrary phase; the check is read-only against committed walk data. **`BYP-9` is preliminary and from an abandoned second run** (ten presses): `dislike`-only held the path at 3 cards for nine consecutive presses, which at 3 cards is **forced 1:1 swapping** — the shape `WHAT-GOOD-LOOKS-LIKE` value 6 says `dislike` should show *least*, and value 7 calls the defect form when sustained. **Explicitly does NOT refute §2.9** — that is in graph-popularity percentile, this is in Spotify listeners, and conflating them is the §2.11/§2.12 error. Its §6 discharges `WHAT-GOOD-LOOKS-LIKE`'s *write down any check that changed your mind* bound: the second opinion revised **five of seven** naive readings, two of them (15M and 14.8M listeners) from "discovery" to "famous outside this listener's geography". Its §7 carries four reproducible URLs whose exclusion counts corroborate the press labels exactly. The pause is intact; nothing proposed. |
