...
PRODUCTION_ACCEPTANCE criteria against the two real 75k artifacts, where the
...
def separating_statistics(graph) -> str:
    ...
    sample = degrees[order[: PRODUCTION_ACCEPTANCE.famous_sample]]
    return (
        f"N={graph.artist_count} E={graph.edge_count} "
        f"median_degree={statistics.median(degrees.tolist())} | "
        f"top-{PRODUCTION_ACCEPTANCE.famous_sample} median degree "
