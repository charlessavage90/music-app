"""ULF- census, offline half: the class per ARCHIVE population, and `ULC-F2`.
...
SOURCES, all local, no network:
  ...
  MB release-group dump  (~18 GiB)  credit split and types, delta only
  MB release dump        (~17 GiB)  track counts, delta only
  MB artist dump         (~17 GiB)  DSP links, Discogs id, Deezer id — full
...
Run from `builder/`:
    UV_LINK_MODE=copy PYTHONIOENCODING=utf-8 uv run python -u \
        analysis/2026-08-05-ulf-census/ulf_census.py
"""
