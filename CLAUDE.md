...
| **What is the next action?** | ⛔ **Phase 1 is PAUSED. Read [`docs/superpowers/2026-07-22-phase1-execution-log-and-graph-defect.md`](docs/superpowers/2026-07-22-phase1-execution-log-and-graph-defect.md) §2 before any Phase 1 work.** A defect was found in the connectivity structure of the **adopted** graph: the most popular artists are among the *least* connected (The Beatles degree 7, Radiohead absent entirely), and the degree-based hub metric therefore does **not** mean "famous." The owner's decision on how to handle it was outstanding when work stopped. That document is one file on purpose — it holds both the defect and the Phase 1 progress record, so the progress cannot be picked up without the context that halted it. |
| **Are the hub / payload figures safe to use?** | **No — see the document above, §2.6.** `hubfrac`, "payload", `hub_penalty` and `w_hub` are all built on top-1%-by-*degree*, which post-`capfix` largely means "member of an insular micro-genre cluster," not "famous artist." Degree and popularity diverge sharply at the top. |
