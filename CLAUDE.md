...
| **Where do bugs, deferrals and task-like work live?** | **GitHub issues, since 2026-09-24** — conventions in [`docs/superpowers/ISSUES.md`](docs/superpowers/ISSUES.md). Each issue is a whole brief (Orca dispatches sessions from them): `owner-decision` means prepare it, never decide it; `agent-ready` means dispatchable now. `NEXT.md` still owns sequencing. |
...
### Long sessions degrade — the tell
...
while omitting skill descriptions, which are. **`closeout` D6 measures the two layers
separately and in different units** — characters for the unconditional one, because it is
written in long single lines that `wc -l` cannot see change in.
